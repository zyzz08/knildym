...
public class DlinkActivity extends Activity implements OnClickListener {
	...
	public void onClick(View arg0) {
		...
		case R.id.bt3:
			...
			break;
		case R.id.bt4:
			result.setText(NetUtil.MD5("software"));
	...
	private String connectSignature(WebCamara dev) {
		...
		try {
			...
			String signature = NetUtil.getSIGNATURE_(NetUtil.MD5_(req
					+ dev.getAuthenticationKeyForSignaling()));
//			String signature = NetUtil.getSIGNATURE(NetUtil.MD5_16(req
//					+ dev.getAuthenticationKeyForSignaling()));
			// 定义获取文件内容的URL
	...
	private String connectDevice(WebCamara dev) {
		...
		try {
			// 定义获取文件内容的URL
			// URL url = new URL("http://baidu.com");
			URL url = new URL("http://"+"admin:software@" + dev.getUPnPIp() + ":"
					+ dev.getUPnPPort() + Constant.CGIPATH_CAMERA_INFO);
			// 打开URL链接
			URLConnection ucon = url.openConnection();
